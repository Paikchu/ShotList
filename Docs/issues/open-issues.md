...
更新日期：2026-09-19。共 **5 项未修复**：**P0 0 项、P1 0 项、P2 5 项**。保留原问题编号，按优先级及编号排序。
...
**修复说明：** `invalidate()` 只置 `isStale` 并取消在途构建，保留已生成的导出包；回收挪到「生成下一个包时」与「应用启动时的 `ExportPackageBuilder.cleanUp()`」两处。这样编辑分镜不会删除仍可能被系统分享面板读取的 zip，迟到的旧构建结果仍会被丢弃。
