...
更新日期：2026-09-21。共 **4 项未修复**：**P0 0 项、P1 0 项、P2 4 项**。保留原问题编号，按优先级及编号排序。
...
**根因证据：** `ExportSession.invalidate()` 里 `if package != nil || isBuilding { isStale = true }` 没有对应的清除分支；`build` 的丢弃分支再显式 `isStale = true`；唯一的 `isStale = false` 在成功分支。旧版本里同一个标记驱动的是独立小节，现象一致（[R-2](delivered-requirements.md#r-2) 验证时发现，不是它引入的）。
...
**来源：** 验证 [R-2](delivered-requirements.md#r-2) 时发现，属于独立根因（`isStale` 只清于成功打包），按 AGENTS.md 单独编号。
