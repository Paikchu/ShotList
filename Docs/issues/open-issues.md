...
更新日期：2026-09-23。共 **6 项未修复**：**P0 0 项、P1 0 项、P2 6 项**。保留原问题编号，按优先级及编号排序。
...
| ☐ | P2-62 | [分镜文字内容指南里的时长只精确到秒（0.7 秒写成 0:01），却要求 AI 以它为准](#p2-62) |
...
### P2-62 · 分镜文字内容指南里的时长只精确到秒（0.7 秒写成 0:01），却要求 AI 以它为准

**验证状态：** 代码级确认；用户真机导出的 9.18Vlog 包数据佐证

**代码位置：** ShotList/Models/Shot.swift · `SLTimecode.text(for:)` 第 372–376 行（四舍五入到整秒）；ShotList/Support/ExportPackageBuilder.swift · 第 1003 行指南的「时长」、第 967 行规则第 5 条「『时长』是该条素材的实际长度……不要臆造未提供的时长」

**问题详情**

**预期行为：** 指南既然要求 AI 按时长估算节奏、且不得臆造，给出的时长精度就要够用。用户写的剪辑风格常按秒以下的粒度提要求（例如「单镜 0.5–2.5 秒」）。

**实际行为：** 时长走的是给人看的 `SLTimecode.text`，四舍五入到整秒：0.718 秒写成「0:01」，0.4 秒写成「0:00」。AI 照它估算，短素材的判断会错。

**根因证据：** 9.18Vlog 包中 `9.18Vlog-08.mov` 实测 0.718 秒，指南写「时长：0:01」；`9.18Vlog-09.mov` 实测 1.507 秒，写「0:02」。`SLTimecode.text` 对小于 0.5 秒的时长返回「0:00」。

**影响范围与定级依据：** 只影响交给 AI 的数值精度，界面与素材不受影响；定为 P2。

**与 [R-15](../requirements/open-requirements.md#r-15) 的关系：** R-15 把精确时长放进 `manifest.json`（毫秒）并把规则移出指南，落地后本条的触发面消失；两者一起做时按 R-15 的产出核对即可，不重复改指南文案。

**复现方法**

1. 准备两段素材：约 0.4 秒和约 1.6 秒，分别导入两个镜头。
2. 「导出」页打包并解压。
3. 看 `分镜文字内容指南.md` 的「三、镜头清单」里这两镜的「时长」。
4. 实际：0:00 与 0:02；预期：精确到 0.1 秒以内，或规则里不再要求以它为准。

**修复状态：** 未修复

**修复说明：** 待修复。给 AI 的时长改用秒（保留小数），或按 R-15 交给 `manifest.json`。
