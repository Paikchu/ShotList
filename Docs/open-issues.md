...
**验证状态：** 代码级确认（未构造删除补偿场景实测）

**代码位置：** ShotList/Views/ClipOptionsSheet.swift · `draftNumber` 初始化（第 62 行）、`flushDraft()`（第 330–352 行）；ShotList/Models/ShotStore.swift · `update(_:)`（第 417–436 行）、`recoverPendingDeletions()`（第 912–917 行）
...
**修复说明：** 待修复；建议只在用户真的动过 Stepper 时才把编号写进 `edited`，或让 `draftNumber` 在 `live.number` 变化且用户未编辑过它时跟随刷新。改动集中在 `ClipOptionsSheet`，不需要动 `ShotStore.update(_:)` 的移动语义。

**验证结果：** 待验证；本轮为代码级确认与全仓写入点检索，未构造删除补偿场景实测。
