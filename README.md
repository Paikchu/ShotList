...
```
分镜导出_20260914/
├── 01-3_无人机缓慢上升，配一句开场旁白.mov   每个镜头的最新一条（拍了多条时带子片段号）
├── 02_手持稳定器横摇，保持水平，速度放慢.mov  只拍一条的镜头不带子片段号
├── 03_手冲壶出水特写，收环境音.mov
├── 备用片段/
...
命名规则是**先分镜号、再子片段号**：镜头拍了多条时，每条（含根目录主素材）都带
「分镜号-第几条」前缀，如 `01-1_…`、`01-2_…`、`01-3_…`，数字越大拍得越晚；
只拍一条的镜头命名为 `01_….mov`，不带后缀。
